...
def test_ok_suporta_acentos_e_espacos(monkeypatch):
    ...
    assert f"q={quote_plus('São Paulo')}" in captured[0]
...
def test_erro_sem_api_key(monkeypatch):
    # Evita que o load_dotenv reponha a chave: define env vazia e recarrega o módulo
    monkeypatch.setenv("OPENWEATHER_API_KEY", "")
    importlib.reload(main_mod)
    # (Opcional) reforço absoluto:
